...
import { insertSpaceAfterPercentSign } from "./optional";
...
class Flow {
  ...
  fixPunctuation(bool: Boolean): Flow {
    if (bool) {
      ...
      p = comma(defaultOption.comma, p);
      // 注意：多个句点变为省略号，和，句点修正，之间的顺序不能颠倒
      p = dots2ellipsis(defaultOption.dots2ellipsis, p);
      p = dot(defaultOption.dot, p);
      p = colon(defaultOption.colon, p);
      p = questionMark(defaultOption.questionMark, p);
      p = bang(defaultOption.bang, p);
      p = semicolon(defaultOption.semicolon, p);
      p = guillemet(defaultOption.guillemet, p);
      p = chineseDash(defaultOption.chineseDash, p);
      p = chineseCommasFold(defaultOption.chineseCommasFold, p);
      p = chineseDotsFold(defaultOption.chineseDotsFold, p);
      p = chineseEllipsisesFold(defaultOption.chineseEllipsisesFold, p);
      p = englishBrackets2ChineseBrackets(
        defaultOption.englishBrackets2ChineseBrackets,
        p
      );
      this.paragraphs = p;
    }
  ...
  fixOthers(bool: Boolean): Flow {
    if (bool) {
      let p = this.paragraphs;
      p = insertSpaceAfterPercentSign(defaultOption.insertSpaceAfterPercentSign, p);
      this.paragraphs = p;
    }
    return this;
  }
